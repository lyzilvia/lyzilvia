...
ATTRIBUTES = 'agaricus-lepiota-names.txt'
...
def parse_attributes():
    with open(ATTRIBUTES, 'r+') as attributes_file:
        for line in attributes_file:
            ...
            pair[-1] = pair[-1].strip()
            g_attributes = pair
            for word in g_attributes:
                g_attributes_dictionary[word]=[]
    with open(TESTDATASET, 'r+') as attributestsset_file:
        for line in attributestsset_file:
                attributes = line.split(',')
                attributes[-1] = attributes[-1].strip()
                for i in range(len(g_attributes)):
                    attr = g_attributes[i]
                    has = g_attributes_dictionary[attr]
                    if attributes[i] not in has:
                        g_attributes_dictionary[attr].append(attributes[i])
    with open(TRAINDATASET, 'r+') as attributestrset_file:
        for line in attributestrset_file:
                attributes = line.split(',')
                attributes[-1] = attributes[-1].strip()
                for i in range(len(g_attributes)):
                    attr = g_attributes[i]
                    ...
                    if attributes[i] not in has:
                        g_attributes_dictionary[attr].append(attributes[i])
    del g_attributes_dictionary['Class']
